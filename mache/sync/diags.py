...
def sync_diags(  # noqa: C901
    ...
    if machine in lcrc_machines and other in lcrc_machines and machine != other:
        raise ValueError(f"You should sync {machine} with itself since files are local")
        ...
        if direction == "from":
            ...
            raise ValueError(
                f"For syncing to work properly, your {other} username is required."
            )
    ...
    try:
        ...
        print("Warning: Some transfer operations failed for public diagnostics.")
    print("")
    print(f"running: {' '.join(private_args)}")
    try:
        subprocess.check_call(private_args)
    except subprocess.CalledProcessError:
        print("Warning: Some transfer operations failed for private diagnostics.")
...
def main():
    ...
    parser.add_argument(
        ...
        "--config_file",
        dest="config_file",
        help="A config file to override default config options for the machine",
    )
    args = parser.parse_args(sys.argv[3:])
    sync_diags(
        args.other, args.direction, args.machine, args.username, args.config_file
    )
